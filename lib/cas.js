                ...
                parser.parseString(response, function(err, result) {
                    ...
                    if (elemSuccess) {
                        ...
                        callback(undefined, true, username, {
                            'username': username,
                            'attributes': parseAttributes(elemSuccess),
...
var parseAttributes = function(elemSuccess)
{
    ...
    if(typeof elemSuccess.getElementsByTagName !== 'function') {
      attrEl = elemSuccess['cas:attributes'];
      if(attrEl === undefined) {
        return;
      }
      if(attrEl.length !== 1) {
        console.error("Do not know how to handle attributes with more than one element.");
        console.error(attrEl);
        return attributes;
      }
      var a = elemSuccess['cas:attributes'][0];
      Object.keys(a).forEach(function(key) {
        attributes[key.replace(/^cas:/,'')] = a[key];
      });
      return attributes;
    }
